...
> **Active Sprint/Phase:** M5 — Polish (portfolio-ready)
...
**Active phase:** M5 — Polish (portfolio-ready)

**Sprint focus:** All four M5 exit criteria are met as of 2026-08-01 — defects at three, ADR set at 001–005 with "complete" finally defined, README case study written, topics set and the fresh-clone quick start actually run. The `addToCart` fix in [#16](https://github.com/Trancend1/commerce-e2e-framework/pull/16) has since cleared the full matrix: nightly run 30683794403 (2026-08-01, sha `1fcaae2`) ran chromium, firefox and webkit at 8 tests each, 8 passed, `check-flake` clean — read from shard logs rather than job conclusions, per the M2 lesson, and webkit counts because BUG-001 surfaced there first. The milestone is **not closed**: §2.2 still wants a Devil's Advocate pass, and that is now the only thing holding it open.

**Orchestrator:** Farhan

**Next:** Critic pass. After that M5 closes and the framework is portfolio-complete; remaining ideas live in the Icebox.
...
**M4 (closed 2026-07-30) — confirmed by the orchestrator 2026-07-29.** Budgets live in `docs/TEST-STRATEGY.md` §3a, the two decisions with real trade-offs in ADR-003 and ADR-004:
...
- [x] Visual regression on the catalog list and checkout step 1, chromium, nightly only, `maxDiffPixelRatio: 0.01`, baselines generated in CI and updated through a PR (ADR-004). Landed in [#12](https://github.com/Trancend1/commerce-e2e-framework/pull/12) and [#13](https://github.com/Trancend1/commerce-e2e-framework/pull/13), green on run 30480555355 with the `visual` suite in the published report. **Deviation from ADR-004:** the `Visual Baselines` workflow uploads an artifact instead of committing and opening the PR itself, because §4.6 forbids bot-authored commits; a human commits and opens it, so baseline changes are still a reviewed diff. **Resolved in M5:** ADR-005 ratifies the artifact route and supersedes ADR-004's update mechanism, so the deviation now lives inside the ADR set rather than as a note here. Choosing the bot-commit route instead would mean revisiting §4.6 in a further ADR

Execution order was k6 → axe → visual, one PR each.

**M5 (exit criteria met 2026-08-01, milestone not yet closed) — mirrors `docs/ROADMAP.md`.**
...
- [x] ADR set complete (001–005). **"Complete" now has a definition**, written into `docs/adr/README.md`: every decision that is significant _and_ hard to reverse gets an ADR; reversible choices with a documented rationale (zero-flaky budget, quarantine mechanics, report retention) stay in the strategy docs, because an ADR set padded with reversible decisions stops signalling which ones matter. The audit against that bar found exactly one gap — ADR-004 instructs the baseline workflow to open its own PR, §4.6 forbids bot-authored commits, and what actually shipped was the artifact route recorded only as a note in §2.5. ADR-005 supersedes that mechanism and pulls the deviation back inside the ADR set
- [x] README case-study section: what this framework caught and what it cost. Written as four parts — the three application defects, the four defects in the framework itself (matrix green while only chromium ran, `retries: 2` hiding a real race, Allure folding four setup executions into a retry badge, an 800ms budget against a 142ms observation), the cost, and a Known gaps section. The framework's own defects are given more space than the application's on purpose: they cost more to find and they are why the suite is trustworthy
- [x] Repo topics and description set; fresh-clone quick start verified. Ten topics set 2026-07-31, description already present. The quick start was then actually run from a clean clone, and it **failed twice** — `npm run test:perf` because k6 is a standalone binary the README never told you to install, and the UI suite because `addToCart()` raced the cart round-trip. Both fixed in [#16](https://github.com/Trancend1/commerce-e2e-framework/pull/16); this criterion earned its tick by breaking, not by passing first time. **Deliberately still not proven: a cold machine.** npm cache, Playwright browsers and Toolshop images were all warm, so the README's "< 5 minutes" claim was removed rather than left standing on a run that never tested it
...
| M4    | Complete (2026-07-30)                               | A check going red is not automatically a finding about the SUT. Of the failures M4 produced, two were mine — the a11y checkout scan timed out because it navigated to the cart without waiting for add-to-cart to land, and the same missing wait had already caused the register flake. Telling your own broken test apart from a real defect is the work; the two real ones became BUG-002 and BUG-003. Second lesson: a threshold calibrated in one condition does not transfer to another. Login p95 is ~134ms at 1 VU and ~406ms at 20, so one number was either unreachable under load or toothless at rest                                                                                                                                                                                                                                                                                                                                                  | Writing ADR-003 before the code paid off — the first a11y scan was red exactly as predicted, so it produced two bug reports instead of a panic. **Open:** (1) promote the a11y scan, and possibly visual, from nightly to the PR gate now that both hold; (2) two k6 budgets are still decoration — search load 81.59ms against 600ms and checkout smoke 32.34ms against 1200ms — tighten once a few nightlies give a trend rather than one sample; (3) npm audit reports 22 dev-dependency vulnerabilities, all newman and eslint transitives, none from axe, untouched here; (4) ADR-004 says the baseline workflow opens its own PR, but §4.6 forbids bot-authored commits, so it uploads an artifact instead — if the bot-commit route is wanted it needs ADR-005, since ADRs are superseded rather than edited; (5) k6 constant-arrival-rate remains the more correct load model if ramping VUs prove too coarse                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| M5    | Exit criteria met (2026-08-01); critic pass pending | Documentation work found a code defect, which is not the order anyone plans. "Verify the quick start" sounded like a formality and turned out to be the most productive task of the milestone: it failed twice and produced [#16](https://github.com/Trancend1/commerce-e2e-framework/pull/16). Second lesson, and the sharper one: the first fix I wrote was wrong. `waitForRequestsToSettle` looked right because it already existed and `CheckoutPage` used it, but measurement showed it returning at 999ms while the request cascade ran to 6.8s — it re-arms a quiet window on request events, and a single in-flight request emits nothing while it hangs, so silence is indistinguishable from completion. Guessing again would have shipped a second fix that changed nothing; instrumented timing is what turned it around. Third: a criterion can be met by being broken — this one earned its tick because it failed, not because it passed first time | **Closed from M4:** carry-forward (4) — ADR-005 ratifies the artifact route and supersedes ADR-004's update mechanism. **Closed in M5:** (d) — the `addToCart` fix cleared the full nightly matrix on run 30683794403 (sha `1fcaae2`, the merge of [#16](https://github.com/Trancend1/commerce-e2e-framework/pull/16)): chromium, firefox and webkit each logged `Running 8 tests` → `8 passed` with `check-flake: no flaky tests`, verified in the shard logs rather than by the nine green job conclusions. **Open, new:** (a) `CheckoutPage.fillBillingAddress` has the same latent quiet-window hole, surviving only because the postcode-lookup cascade is chatty enough to keep re-arming it — and that is the cascade BUG-001 describes; (b) `workers` is unpinned locally, so a high-core machine oversubscribes and produces failures unrelated to any real defect, as 8 workers did here; (c) the deterministic race harness was a proof, not a shipped test — worth adding as a regression test, but its 3000ms delay must become a named budget per §4.2. **Open, carried:** M4's (1), (2), (3) and (5); the trend precondition on (2) is now satisfied by two green post-M4 nightlies (30512555679, 30604025670)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
