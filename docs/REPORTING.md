...
- A Pages deploy replaces the whole site, so the previous one is carried forward as a `published-site` workflow artifact, restored by the next run before it assembles. An artifact rather than a `gh-pages` branch, so the nightly never writes to the repo's commit history. Assembly and retention live in `scripts/assemble-report-site.mjs`, which rebuilds the root from scratch each time — Allure emits hashed asset names, so overwriting in place would accumulate every past run's orphaned assets
